...
class CrosswordCreator():
    ...
    def consistent(self, assignment):
        ...
        if assignment:
            ...
            # check every value is the correct length (unary constraints)
            for key,val in assignment.items():
                if len(val) != key.length:   
                    return False
      
            # check there are no conflicts between neighboring variables (binary constraints)
            for key,val in assignment.items():
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        domains = {}
        unassigned_neighbors = [var for var in self.crossword.neighbors(var) if var not in assignment.keys()]
        for domain_var in self.domains[var]:
            num = 0 
            for neighbor in unassigned_neighbors:
                if domain_var in self.domains[neighbor]:
                    num += 1
            domains[domain_var] = num
        
        #  ascending 
        return sorted(domains, key=lambda key: domains[key])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return values.
        """
        remaining_values_dic = {}
        unassigned_variables = [var for var in self.crossword.variables if var not in assignment.keys()]
        
        for var in unassigned_variables:
            remaining_values_dic[var] = len(self.domains[var])
        
        min_val = min(remaining_values_dic.values())
        min_keys = [key for key,val in remaining_values_dic.items() if val == min_val]
        
        # If there is a tie, choose the variable with the highest degree
        if len(min_keys) >  1:
            tie_variables = {}
            for var in min_keys:
                tie_variables[var] = len(self.crossword.neighbors(var))
            #  descending 
            sorted_list = sorted(tie_variables.items(), key=lambda x: x[1],reverse=True)
            return sorted_list[0][0]
         
        #  ascending 
        sorted_list = sorted(remaining_values_dic.items(), key=lambda x: x[1])
        return sorted_list[0][0]
